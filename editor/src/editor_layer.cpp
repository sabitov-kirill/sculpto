...
class camera_behaviour: public scl::scene_object_behaviour
{
    ...
    {
        auto &camera = GetComponent<scl::camera_controller_component>().Camera;
...
void scl::editor::editor_layer::OnInit()
{
    ...
    EditorScene->SetIsHDR(true);
    EditorScene->SetIsBloom(true);
    ...
    projector.AddComponent<directional_light_component>(vec3 { -0.5, -1, 0 }, vec3 { 1 }, true, 10.0f, 100.0f, 10000, 10000);

    auto model = EditorScene->CreateObject("Model, loaded from file");
    model.AddComponent<mesh_component>(assets_manager::LoadMeshes("assets/models/robot/scene.gltf"));
    ...
    camera rend_camera { camera_projection_type::PERSPECTIVE };
    ...
    SceneHierarchyWindow = CreateShared<scene_hierarchy_window>(EditorScene.get(), SceneObjectConfigWindow->ConfiguringObject);
...
void scl::editor::editor_layer::OnEvent(event &Event)
{
